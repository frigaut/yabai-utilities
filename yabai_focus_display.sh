#!/bin/bash
# This script will focus the closest space in the requested display that is focusable
# that is, that has at least a window
# yabai_focus_display.sh

# What's the current display and space?
curdis=$(yabai -m query --displays --display $1 | jq '.index')
...
yabai -m query --spaces --display | jq '.[] | select (."is-visible") | .index'
